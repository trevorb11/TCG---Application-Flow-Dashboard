...
import { syncApplicationToSalesforce, syncClcAttributionToSalesforce, syncDecisionToSalesforce, syncUwSubmissionToSalesforce, syncAiSnapshotToSalesforce, promoteOpportunityToUnderwriting, recordEmailClickInSalesforce, recordGhlEmailReplyInSalesforce, syncRepAssignmentToSalesforce } from "./services/salesforce";
...
<html><body style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;">
  ...
  const GHL_EMAIL_REPLY_WEBHOOK_KEY = process.env.GHL_EMAIL_REPLY_WEBHOOK_KEY || EMAIL_CLICK_WEBHOOK_KEY;
  ...
  // GHL workflow "Outbound Webhook" step fired for an inbound email. The
  // workflow's custom fields may arrive at the root or inside customData, so
  // accept both shapes. Full message bodies are never written to application
  // logs; only identifiers and the Salesforce result are logged.
  app.get("/api/webhooks/ghl/email-reply/health", (_req: Request, res: Response) => {
    res.json({ ok: true, service: "ghl-email-reply-to-salesforce" });
  });

  app.post("/api/webhooks/ghl/email-reply", async (req: Request, res: Response) => {
    const suppliedKey = String(req.query.key || req.header("x-tcg-webhook-key") || "");
    const expectedKey = GHL_EMAIL_REPLY_WEBHOOK_KEY;
    const supplied = Buffer.from(suppliedKey);
    const expected = Buffer.from(expectedKey);
    const authorized = supplied.length === expected.length && timingSafeEqual(supplied, expected);
    if (!authorized) return res.status(403).json({ error: "bad key" });

    try {
      const b: any = req.body || {};
      const c: any = b.contact || {};
      const d: any = b.customData || b.custom_data || {};
      ...
      const contactEmail = pick(d.contact_email, d.email, b.email, c.email);
      const fromEmail = pick(d.from_email, d.fromEmail, b.from_email, b.fromEmail, m.from, contactEmail);
      const fromName = pick(d.from_name, d.fromName, b.from_name, b.fromName, b.full_name, c.full_name, c.fullName);
      const subject = pick(d.subject, b.subject, m.subject) || "(no subject)";
      const latestMessage = pick(d.latest_message, d.latestMessage, b.latest_message, b.latestMessage, m.body);
      const fullThread = pick(d.full_thread, d.fullThread, b.full_thread, b.fullThread, latestMessage);
      const ghlMessageId = pick(d.message_id, d.messageId, b.message_id, b.messageId, m.id);
      const ghlContactId = pick(d.ghl_contact_id, d.ghlContactId, b.ghl_contact_id, b.contact_id, b.contactId, c.id);
      const toAddress = pick(d.to_email, d.toEmail, b.to_email, b.toEmail, m.to);
      const receivedAt = pick(d.received_at, d.receivedAt, b.received_at, b.date_created, m.dateAdded, m.date_added);

      const missing: string[] = [];
      if (!contactEmail || !contactEmail.includes("@")) missing.push("contact email");
      if (!fromEmail || !fromEmail.includes("@")) missing.push("from email");
      if (!ghlMessageId) missing.push("message_id");
      if (!fullThread) missing.push("full_thread/latest_message");
      ...
      if (String(req.query.dry_run || "") === "1") {
        return res.json({
          ...
          parsed: {
            ...
            hasFullThread: Boolean(fullThread),
          },
        });
      }

      const out = await recordGhlEmailReplyInSalesforce({
        contactEmail,
        fromEmail,
      ...
      console.log(`[GHL-EMAIL-REPLY] ${ghlMessageId} ${contactEmail} -> ${out.result}${out.error ? ` (${out.error})` : ""}`);
      if (out.result === "error") return res.status(500).json(out);
      return res.json({ synced: out.result === "created" || out.result === "duplicate", ...out });
    } catch (e: any) {
      console.error("[GHL-EMAIL-REPLY] handler error:", e?.message || e);
